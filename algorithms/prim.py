# 1. Make adjacency list (optional)
# 2. Set first vertex to taken, add neighbours to priority queue
...
def process(u, taken, AL, pq):                      # set u as taken and enqueue neighbors of u
    taken[u] = 1
    for v, w in AL[u]:
        if (not taken[v]):                          # v is not yet taken as other vertex's neighbor
...
def prim():
    ...
    taken = [0 for _ in range(V)]                   # to avoid cycle, no vertex is taken
